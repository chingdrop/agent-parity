...
Splunk is a *sink* here, never the system of record — the Django database
stays authoritative, and this module only ships already-classified results
for orgs that centralize alerting in Splunk. Two deliberate choices follow
from that:
...
This package has no Django imports, so callers hand us plain delta dicts —
the ORM diffing lives in the dashboard app.
"""
