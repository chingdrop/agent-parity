...
* Remote execution: Remote Script Orchestration — upload the script to the
  ...
  **These RSO mechanics (``_headers``/``_live_deploy_and_run``) live in
  ``SentinelOneRSOMixin`` below**; ``SentinelOneConnector`` adds the
...
from agent_parity.connectors.base import AgentConnector, ConnectorError, parse_timestamp, register_connector
...
class SentinelOneRSOMixin:
    ...
    polling all come from the ``VendorConnector``-derived base the
    ...
    class — never this mixin. For the same reason it doesn't actually
    inherit ``VendorConnector`` either (that would force a specific MRO on
    the class it's mixed into); each method instead declares an explicit ``_ConnectorLike``
    ...
    def _live_deploy_and_run(
        ...
        # object-storage handoff — see ``agent_parity.script_runner``).
