...
**The MinIO and Celery smoke tests are manual.** [`docker/smoke_test.sh`](../docker/smoke_test.sh) needs Docker and runs
neither in `uv run pytest` nor in CI, so real MinIO and a real Celery chord are only checked when someone runs it. See
