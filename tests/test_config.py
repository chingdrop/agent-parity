...
def test_global_scope_with_multiple_accounts_requires_an_explicit_choice():
    ...
    config = replace(load_config(), clients={**load_config().clients, "test": client})
...
def test_global_scope_with_no_accounts_configured_resolves_to_empty_dict(monkeypatch):
    ...
    # Unset env vars still leave the accounts *named* (config.yaml declares
    # them) — to test the "zero accounts at all" case, build a client
    # against a vendor with an empty accounts dict directly.
    vendor = VendorConfig(name="sentinelone", scope="global", accounts={})
    client = _client(("sentinelone",))
    config = AppConfig(stale_days=14, vendors={"sentinelone": vendor}, clients={"test": client}, storage=load_config().storage)
